/*
 * Decompiled with CFR 0.152.
 * 
 * Could not load the following classes:
 *  org.json.JSONObject
 */
package dji.sdk.keyvalue.value.flightcontroller;

import dji.jni.JNIProguardKeepTag;
import dji.sdk.keyvalue.value.ByteStream;
import dji.sdk.keyvalue.value.base.DJIValue;
import dji.sdk.keyvalue.value.flightcontroller.CompassSensorState;
import dji.sdk.keyvalue.value.flightcontroller.IMUCalibrationState;
import dji.sdk.keyvalue.value.flightcontroller.IMUSensorState;
import org.json.JSONObject;

public class IMUState
implements DJIValue,
JNIProguardKeepTag,
ByteStream {
    Integer index;
    IMUSensorState gyroscopeState;
    IMUSensorState accelerometerState;
    Integer calibrationProgress;
    IMUCalibrationState calibrationState;
    CompassSensorState compassSensorState;
    Integer compassSensorValue;
    Double gyroscopeBias;
    Double accelerometerBias;

    public IMUState() {
    }

    public IMUState(Integer n, IMUSensorState iMUSensorState, IMUSensorState iMUSensorState2, Integer n2, IMUCalibrationState iMUCalibrationState, CompassSensorState compassSensorState, Integer n3, Double d2, Double d3) {
    }

    public static IMUState fromJson(String string) {
        return null;
    }

    @Override
    public JSONObject toJson() {
        return null;
    }

    @Override
    public int serializedLength() {
        return 0;
    }

    @Override
    public int toBytes(byte[] byArray, int n) {
        return 0;
    }

    @Override
    public byte[] toBytes() {
        return null;
    }

    @Override
    public int fromBytes(byte[] byArray, int n) {
        return 0;
    }

    public Integer getIndex() {
        return null;
    }

    public void setIndex(Integer n) {
    }

    public IMUSensorState getGyroscopeState() {
        return null;
    }

    public void setGyroscopeState(IMUSensorState iMUSensorState) {
    }

    public IMUSensorState getAccelerometerState() {
        return null;
    }

    public void setAccelerometerState(IMUSensorState iMUSensorState) {
    }

    public Integer getCalibrationProgress() {
        return null;
    }

    public void setCalibrationProgress(Integer n) {
    }

    public IMUCalibrationState getCalibrationState() {
        return null;
    }

    public void setCalibrationState(IMUCalibrationState iMUCalibrationState) {
    }

    public CompassSensorState getCompassSensorState() {
        return null;
    }

    public void setCompassSensorState(CompassSensorState compassSensorState) {
    }

    public Integer getCompassSensorValue() {
        return null;
    }

    public void setCompassSensorValue(Integer n) {
    }

    public Double getGyroscopeBias() {
        return null;
    }

    public void setGyroscopeBias(Double d2) {
    }

    public Double getAccelerometerBias() {
        return null;
    }

    public void setAccelerometerBias(Double d2) {
    }

    public String toString() {
        return null;
    }
}

